...
import {
  activateReviewerScoringFormula,
  createReviewerScoringFormula,
  getReviewerFormulaThresholds,
  getReviewerFormulas,
} from "@/lib/api/admin";
...
export function AdminRankingFormulaPage() {
  ...
  async function handleCreate() {
    ...
    setCreateError(null);
    try {
      await createReviewerScoringFormula({
  ...
  async function handleActivate(id: string) {
    setActivatingId(id);
    try {
      await activateReviewerScoringFormula(id);
      void load();
    } finally {
  ...
  return (
    <div className="flex flex-col gap-6">
      <AdminPageHeader
        ...
        description="Formula for calculating reviewer scores used to generate ranking snapshots. Active formula is currently applied."
        actions={
      ...
      <AdminDataTable
        ...
        emptyDescription="No scoring formulas have been created."
      />
      ...
      <AdminConfirmDialog
        ...
        title="Create scoring formula"
        description="New formula will be in Inactive status. Activate to apply ranking calculation."
        confirmLabel="Create"
      ...
      >
        {thresholds.length ? (
          ...
          <p className="text-sm text-muted">This formula has no thresholds configured.</p>
        )}
      </AdminDetailDialog>
    </div>
  );

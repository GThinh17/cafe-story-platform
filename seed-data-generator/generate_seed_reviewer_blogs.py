"""
Sinh SQL seed 100 blog cá nhân của 40 reviewer (20 rev × 2 blog + 20 rev × 3 blog).

Follow rule BE:
- author_user_id = reviewer user_id
- page_id = NULL (blog cá nhân, không thuộc cafe page)
- region_id = user.region_id (BE require không null, dùng region của user)
- content NOT NULL — caption tiếng Việt góc nhìn reviewer
- imageUrls (blog_images): 2 ảnh Cloudinary/blog
- status = PUBLISHED

created_at rải trong 60 ngày qua với interleaved slots — reviewer 1-40 và blog 1-3
được xen kẽ để feed đa dạng theo thời gian.

Chạy: python generate_seed_reviewer_blogs.py
Output: output/2026-07-16_seed_100_reviewer_blogs_cantho.sql
"""
...
OUTPUT_PATH = Path(__file__).parent / "output" / "2026-07-16_seed_100_reviewer_blogs_cantho.sql"
URLS_PATH = Path(__file__).parent / "reviewer_blog_image_urls.json"

TOTAL_REVIEWERS = 40
COUNT_2_BLOG = 20
COUNT_3_BLOG = 20
TOTAL_BLOGS = COUNT_2_BLOG * 2 + COUNT_3_BLOG * 3  # 100

SPREAD_DAYS = 60
MINUTES_PER_SLOT = (SPREAD_DAYS * 24 * 60) // TOTAL_BLOGS  # 864
...
def _load_urls() -> Dict[str, Dict[str, List[str]]]:
    if not URLS_PATH.is_file():
        raise SystemExit(f"Không tìm thấy {URLS_PATH.name} — chạy upload_reviewer_blog_images.py trước.")
...
def _emit_reviewer_block(rev_index: int, urls: Dict[str, List[str]], slot_base_ref: List[int]) -> str:
    """Sinh SQL cho 1 reviewer với N blog."""
    n_blogs = _blogs_for(rev_index)
    lines: List[str] = [
        f"    -- === Reviewer #{rev_index:02d} ({n_blogs} blog) ===",
    ...
    for blog_idx in range(1, n_blogs + 1):
        ...
        # Pick caption template deterministic theo (rev_index, blog_idx)
        caption_template = caption_template_for(
            blog_idx, f"rev{rev_index}_blog{blog_idx}"
        )
        # Template có {name} placeholder — split để SQL concat với v_user_full_name
        left, right = caption_template.split("{name}")

        lines.append(
            f"    -- Blog #{blog_idx} (slot {blog_slot}, {offset_min} min ago)"
        )
        # v_user_full_name chỉ cần SELECT 1 lần đầu tiên cho mỗi reviewer,
        # nhưng đặt trong mỗi block cho dễ đọc + độc lập nếu cắt bớt block
        if blog_idx == 1:
            lines.append(
...
def _emit_sql(urls: Dict[str, Dict[str, List[str]]]) -> str:
    header = f"""-- Seed {TOTAL_BLOGS} blog cá nhân cho {TOTAL_REVIEWERS} reviewer Cần Thơ.
...
BEGIN
    ...
    FROM (
        ...
        OFFSET 20 LIMIT {TOTAL_REVIEWERS}
    ) sub;
    IF v_reviewer_ids IS NULL OR array_length(v_reviewer_ids, 1) < {TOTAL_REVIEWERS} THEN
        RAISE EXCEPTION 'Chỉ pick được %/{TOTAL_REVIEWERS} reviewer, cần đủ {TOTAL_REVIEWERS}.',
                        coalesce(array_length(v_reviewer_ids, 1), 0);
    END IF;

"""

    # Interleaved slot counter (shared across all reviewers)
    slot_ref = [0]
    body_parts = [

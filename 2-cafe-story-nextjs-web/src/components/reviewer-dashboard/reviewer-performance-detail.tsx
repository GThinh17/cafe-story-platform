...
export function ReviewerPerformanceDetail() {
  ...
  const bestMonth = mockReviewerPerformance.reduce((best, item) =>
    item.score > best.score ? item : best,
  );
  const maxScore = Math.max(...mockReviewerPerformance.map((item) => item.score), 1);
      ...
      <Card className="p-5">
        ...
        <div className="mt-5 overflow-x-auto">
          <table className="w-full min-w-[640px] text-left text-sm">
            ...
            <tbody>
              {mockReviewerPerformance.map((item) => (

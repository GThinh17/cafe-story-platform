...
export function ReviewerEarningsDetail() {
  const current = mockReviewerPayouts[0];
  const allTimeTotal = mockReviewerPayouts.reduce(
    (total, payout) => total + payout.totalAmount,
    0,
  );
  const totalLikeAmount = mockReviewerPayouts.reduce(
    (total, payout) => total + payout.likeAmount,
    0,
  );
  const totalShareAmount = mockReviewerPayouts.reduce(
    (total, payout) => total + payout.shareAmount,
    0,
  );
  const totalCommentAmount = mockReviewerPayouts.reduce(
    (total, payout) => total + payout.commentAmount,
    0,
  );
  const maxAmount = Math.max(...mockReviewerPayouts.map((item) => item.totalAmount), 1);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Card className="p-5">
          <p className="text-sm font-black text-muted">Current payout</p>
          <p className="mt-3 text-3xl font-black text-primary">
            {formatVnd(current.totalAmount)}
          </p>
        </Card>
        <Card className="p-5">
          <p className="text-sm font-black text-muted">All mock months</p>
      ...
      <Card className="p-5">
        ...
        <div className="mt-5 overflow-x-auto">
          <table className="w-full min-w-[920px] text-left text-sm">
            ...
            <tbody>
              {mockReviewerPayouts.map((payout) => (
                <tr className="border-t border-line-soft" key={payout.id}>
                  ...
                  <td className="py-3 pr-3 text-muted">
                    {formatVnd(payout.likeAmount)}
                  </td>
                  <td className="py-3 pr-3 text-muted">
                    {formatVnd(payout.shareAmount)}
                  </td>
                  <td className="py-3 pr-3 text-muted">
                    {formatVnd(payout.commentAmount)}
                  </td>
